...
\section*{La trama come istanza}

Vari momenti della Parte~I trovano qui la propria derivazione.

\emph{La disequazione del jazz bar} --- $(A,G) \not< (F,G)$ eppure $(F,G)$ posizione preferita --- è il Principio[Tipo del verbo preferire]: due grammatiche, stessa parola, due lavori diversi. In $\Lt$ ``preferito'' presuppone l'ordine cumulativo in $\KSigma$; in $\LP$ è il ruolo che la configurazione attiva.

\emph{La sequenza $F \to T \to N$} --- copione identico con argomenti diversi nello stesso ruolo --- è il Principio[Sostituibilità degli argomenti]. Non scelta di qualità, non confronto: $\Gs$-invarianza di $\Kappaa$. Gli argomenti sotto soglia sono parametri della proiezione, non dati distinti.

\emph{Il pianto delle 21:30 seguito dall'ilarità delle 21:35} --- stessa persona, sequenza ininterrotta, due affetti incompatibili sotto $\Lt$ --- è la memoria markoviana di $\LP$ (cap.~\emph{Lo spettro}, asimmetrie). $\LP$ tiene la configurazione corrente, niente coordinate cumulative; ogni configurazione è intera in sé, nessun carry-over richiesto fra una e la successiva. Non dissociazione, non mascheramento: forma del codominio della carica.

\emph{Il timing della rivelazione del ragazzo} --- detta solo dopo che la sera era stata spesa --- non è strategia. È principio di scelta minima (cap.~\emph{Dinamica e tempo}): il passo locale processa la configurazione corrente per minimizzare il costo immediato, e non c'è funzionale globale che pianifichi destinatari futuri. $\LP$ non guarda avanti, perché la sua dinamica non ammette traiettorie come oggetti confrontabili.

\emph{La foto a $N$ in doccia} --- mossa che un pianificatore strategico non avrebbe fatto --- è la stessa scelta minima: il passo emette il segnale alto-temperatura verso $N$ richiesto dalla configurazione corrente, e gli effetti collaterali a valle (la foto che arriva ad $A$) non sono computati perché un computo del genere richiederebbe un funzionale globale sulla traiettoria, che $\LP$ non possiede.

Nessuno di questi episodi richiede strategia, malafede o calcolo. Sono esecuzione regolare di una legge applicata passo per passo a un pool che evolve. La coerenza apparente della sequenza --- che da $\Lt$ può sembrare disegno --- è proprietà interna della legge stessa, non firma di un'intenzione che la precede.
