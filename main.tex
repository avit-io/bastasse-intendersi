...
Il falso testimone è bidirezionale. Ma il sollevamento nella fibra sbagliata ha forma diversa nei due sensi.

Quando $O$ in regime con codominio libero (es.~$\Lt$) vede output di $L'$ con codominio quoziente (es.~$\LP$), un sollevamento candidato $\tilde{L}_O(c) \in \mathbf{D}_{\Lt}$ aggiunge struttura assente: ricostruisce storia, deposito, ordine accumulativo dove $L'$ non li produce. Falso testimone di aggiunta.

Quando $O$ in regime con codominio quoziente (es.~$\LP$) vede output di $L'$ con codominio libero (es.~$\Lt$), un sollevamento candidato $\tilde{L}_O(c) \in \mathbf{D}_{\LP}$ appiattisce struttura presente: collassa la storia in una classe configurazionale, dimentica l'ordine. Falso testimone di collasso.

Sono errori di tipo opposto --- uno aggiunge, l'altro dimentica --- ed entrambi sono in buona fede strutturale.

\paragraph{Riconoscibilità unilaterale.} L'asimmetria importante è al livello successivo. Solo i regimi con aggiunto sinistro possono confrontare il sollevamento scelto con $F_L(\mathrm{SM})$ e classificare la differenza come errore di fibra. Gli altri lo commettono senza poterlo sapere. Il falso testimone è bidirezionalmente reale, ma la sua riconoscibilità è unilaterale.
...
Il pianista che ripete la scala è $\Lo$ sano: il contesto ammette rituale, il rituale è la struttura del lavoro. Il controllore del gas che torna dieci volte a verificare prima di uscire è $\Lo$ patologico: il contesto non ammette rituale, il rituale è imposto a forza. Stessa legge, contesti opposti; la patologia non è proprietà dell'individuo, è proprietà della coppia. Relazionale, non assoluta.
