...
Due dati esterni alla legge, non leggibili da $\mathbf{C}$ nuda, vanno dichiarati apertamente. Il primo è la proiezione configurazionale $\rho_{\mathrm{conf}} : \mathcal{I}_c \to P$, che associa a ogni interazione la configurazione del pool in cui occorre. Il secondo è la mappa pre-riflessiva $\Sogno : P \to \mathbb{R}_{\geq 0}$, che a ogni argomento del pool associa l'intensità con cui la persona ne è orientata: tropismi, attrazioni, gradi di vivezza che precedono la legge. $\LP$ riceve $\Sogno$ come ingresso, lo filtra via $-\lambda \cdot \mathrm{threat}$ (per $\lambda \gg 1$ la threat alta cancella in ingresso il contributo della sezione), e proietta la configurazione filtrata su $P/\Gs$. La legge dice cosa il filtro fa; cosa $\Sogno$ contenga --- perché la persona sogni \emph{questi} argomenti e non altri --- non è formulabile nella grammatica della legge. Si torna sull'opacità di $\Sogno$ al bordo del logos.

\section*{Corollario: sostituibilità degli argomenti}

Dalla $\Gs$-invarianza di $\Kappaa$ segue subito una conseguenza fenomenologica.

\begin{principio}[Sostituibilità degli argomenti]
In $\LP$, due argomenti $x, y \in P$ con $\mathrm{threat}(x), \mathrm{threat}(y) < \theta$ che occupano lo stesso ruolo nella configurazione corrente stanno nella stessa orbita di $\Gs$. La carica $\Kappaa(P)$ è invariante per la permutazione $\sigma_{xy}$ che li scambia: $\Kappaa(\sigma_{xy} \cdot P) = \Kappaa(P)$. L'esecuzione di $\LP$ sopra una configurazione in cui $x$ occupa il ruolo $R$ è strutturalmente identica all'esecuzione sopra una configurazione in cui $y$ lo occupa --- gli argomenti sono parametri della proiezione, non dati distinti.
\end{principio}

\emph{Derivazione.} Per definizione, $\Gs = \Sym(\{c \in P : \mathrm{threat}(c) < \theta\})$, e $\Kappaa = P/\Gs$ è invariante per costruzione. Una permutazione che scambia due argomenti sotto soglia agisce dentro $\Gs$; il quoziente non la vede. $\square$

La sequenza $F \to T \to N$ della Parte~I --- copione che gira identico con argomenti diversi nello stesso ruolo --- è esattamente questo. Tre istanze della stessa orbita di $\Gs$, lette dalla stessa $\Kappaa$ senza che le differenze tra $F$, $T$, $N$ entrino nella legge. Non scelta di qualità relazionale, non confronto: $\Gs$-invarianza.
...
\section*{Inganno su sé stessi}

Il falso testimone non inganna solo sull'altro. Inganna su sé stessi.

Quando $\Lt$ solleva l'output di $\LP$ aggiungendo struttura narrativa, ricostruisce non solo un piano interno dell'altra --- agente, intenzioni, calibrazione sul destinatario --- ma anche, automaticamente, un \emph{giudizio} che quell'agente starebbe pronunciando su chi guarda. Il sollevamento porta con sé il proprio destinatario: se l'altra è $\Lt$, allora pesa, accumula, calibra, e una delle cose che pesa è la posizione di $A$ nella sua traiettoria. Il fantasma di giudizio è ricostruzione canonica via $F_{\Lt}$, non scelta.

$A$ legge quindi nell'output di $G$ un verdetto sul proprio valore. La disequazione del bar diventa ``$F$ è preferito a me'' --- proposizione che presuppone un confronto su una scala, e una valutazione di $A$ rispetto a $F$ su quella scala. Ma $\LP$ non aveva giudicato: aveva assegnato ruoli dalla configurazione corrente. Il verdetto non era stato emesso. Era stato sollevato come artefatto del falso testimone.

Conseguenza: $A$ impara di sé qualcosa che nessuno gli ha mai detto. Si porta dietro per anni una valutazione che non esiste come atto, perché il regime che l'avrebbe potuta emettere non ha la grammatica per emettere valutazioni di quel tipo. È il livello più sottile, e più resistente, del falso testimone: non solo proietta un agente narrativo dove non c'è; proietta, dentro quell'agente, un giudizio su sé che l'osservatore prende come reale.

La ritrattazione che questo libro esegue --- formalmente solo dell'attribuzione di regime --- ha quindi un secondo effetto strutturale, che vale la pena nominare: ritira anche il verdetto fantasma. Non perché qualcuno l'avesse mai pronunciato, ma perché il sollevamento che lo produceva è stato classificato come errore di fibra. Quando $\Lt$ smette di sollevare $\LP$ in $\mathbf{D}_{\Lt}$, il verdetto che credeva di leggere su di sé cessa di esistere come oggetto.
...
\section*{Gli episodi, in tre prospettive}

Ogni episodio della Parte~I si lascia ripercorrere in tre passi: come si presenta a $A$ (lettura in $\Lt$ dell'epoca), come si produce in $G$ (lettura strutturale, vent'anni dopo), dove sta il disallineamento che genera il fraintendimento. Il cap.~successivo zooma sul pianto al bar; qui il resto.

\subsection*{Il magone alla porta, ad Asiago}

\emph{Letta in $\Lt$.} Quando lei apre la porta, $A$ è attraversato da un trasalimento. Postura senza armatura --- ``apertura senza corazza''. Da $\Lt$ il magone è atto generativo che deposita pesante in $\KSigma$: $\Phii$ accumula, il contesto $c_{AG}$ riprende.

\emph{Prodotta in $\LP$.} $A$ entra nella configurazione corrente del pool come argomento nuovo. Lei lo registra, la configurazione cambia da ``$A$ assente'' a ``$A$ presente sotto soglia''. Niente di $\Lt$ accade dal lato di $G$ --- non c'è traiettoria che riprende, c'è transizione di classe del pool.

\emph{Disallineamento.} L'asimmetria è già completa al primo passo, prima ancora che qualcuno parli. $A$ vede ricominciare qualcosa che, dal lato $G$, non ha mai avuto la forma che ricomincerebbe.

\subsection*{La disequazione del bar}

\emph{Letta in $\Lt$.} $(A,G) \not< (F,G)$ è paradosso. $F$ è ``due morti che camminano'' --- nessun comportamento accumulato che giustifichi preferenza --- e tuttavia $F$ è preferito. Il confronto cumulativo non spiega. $A$ resta dentro il paradosso per anni.

\emph{Prodotta in $\LP$.} $F$ porta il tag \texttt{ragazzo} che la configurazione corrente del pool gli ha assegnato. $A$ porta un tag diverso (nuovo, sotto soglia). ``Preferito'' in $\LP$ non è confronto --- è classificazione: $F$ occupa il ruolo che la configurazione attiva.

\emph{Disallineamento.} Il Principio[Tipo del verbo preferire] (cap.~\emph{Lo spettro}). La parola ``preferire'' suona la stessa, lavora in due grammatiche. Il paradosso che $A$ vive è la differenza di tipo, registrata correttamente come paradosso da $\Lt$ ma impossibile da risolvere senza cambiare grammatica.

\subsection*{Il giardino, l'ammorbidimento, la madre richiama}

\emph{Letta in $\Lt$.} Dopo la chiusura della sera, lei si avvicina in giardino con abbraccio e trama abbondante. Avvicinamento senza che la chiusura sia stata revocata. La madre interrompe dal balcone --- chiusura preventiva di qualcosa che non sta succedendo. $\Lt$ legge ambiguità calcolata, o scena montata.

\emph{Prodotta in $\LP$.} La sera prima ha chiuso una configurazione del pool. La mattina è configurazione nuova: ``vieni in centro'' apre nuova classe, senza che la sera prima debba essere portata. La madre richiama quando il pool corrente tocca un limite che la configurazione esterna (famiglia) non ammette --- non risponde a un piano, risponde al pool del momento.

\emph{Disallineamento.} La memoria markoviana di $\LP$. $\Lt$ presuppone che le chiusure si trasportino fino a revoca esplicita; $\LP$ non trasporta nulla. Quello che $\Lt$ legge come ``ambiguità'' è normalità dal lato dell'altra grammatica.

\subsection*{Tommaso al parco: la minaccia di $F$}

\emph{Letta in $\Lt$.} Tommaso porta il messaggio: ``$F$ vi ha scoperti, se non la smetti viene a picchiarti.'' $\Lt$ legge manipolazione strategica: lei ha orchestrato l'uso di $F$ come argomento di minaccia, Tommaso come messaggero. Pianificatore-per-me al lavoro.

\emph{Prodotta in $\LP$.} La configurazione corrente del pool ha bisogno di assegnare il tag \texttt{minaccia} verso $A$ --- la classe richiede quella mossa per restare regolare. $F$ è argomento sotto soglia disponibile con il ruolo storico (era stato \texttt{ragazzo}, è ancora nel pool). $\Gs$-sostituibilità: il ruolo è quello che conta, l'argomento ($F$) è parametro permutabile.

\emph{Disallineamento.} Principio[Sostituibilità degli argomenti] (cap.~\emph{Lo spettro}). $F$ qui non è ``$F$ come persona che minaccia $A$'' --- è un argomento del pool messo in posizione di minaccia dalla configurazione corrente. $\Lt$ legge agency individuale dove c'è esecuzione di ruolo.

\subsection*{Vicenza, sei mesi dopo: l'ariete}

\emph{Letta in $\Lt$.} Sei mesi senza atti su $c_{AG}$ dal lato $A$: $\|\delta\| = 0$, $\tau^{\Lt}$ fermo. Il monologo su $T$ chiuso con ``ho casa libera'' arriva sulla traiettoria ferma di $A$ come atto successivo del jazz bar. La risposta dell'ariete è metacognizione di $\Lt$ in atto: $A$ pronuncia la letteralità della scena per uscirne, sapendo che la pronuncia stessa la svuota.

\emph{Prodotta in $\LP$.} $\tau^{\LP}$ è avanzato per molte transizioni di classe in sei mesi: $T$ è entrato nel ruolo \texttt{ragazzo}, $F$ è scivolato, altri argomenti si sono mossi. La configurazione corrente riattiva $A$ come argomento sotto soglia. ``Casa libera'' è alta-temperatura verso $A$, nuova esecuzione, non ripresa.

\emph{Disallineamento.} I due tempi non si incollano (cap.~\emph{Dinamica e tempo}). $A$ continua da dove $\tau^{\Lt}$ si era fermato; $G$ è in nuovo passo del proprio contatore di salti. Le due frasi della stessa scena fisica vivono in due tempi diversi: ``casa libera'' è $d\tau^{\LP} = 1$, l'ariete è continuazione di $\tau^{\Lt}$ con peso elevato. Stessa stanza, due cronologie incompatibili.

\subsection*{Canottaggio: la foto a $N$}

\emph{Letta in $\Lt$.} Un pianificatore strategico non manda quella foto a $N$ amico di $A$. La mossa è incoerente con la coerenza apparente del resto della sequenza. Stupidità? Troppo sofisticata altrove. Strategia di ingelosimento? Manca canale, manca controllo della consegna, manca verifica dell'effetto. Resta solo: \emph{egocentrismo dell'osservatore}, che attribuisce l'atto a sé stesso come destinatario.

\emph{Prodotta in $\LP$.} Scelta minima locale (cap.~\emph{Dinamica e tempo}). La configurazione corrente del pool richiede segnale alto-temperatura verso $N$. La mossa minimizza il costo locale; gli effetti collaterali a valle --- la foto che arriva in doccia ad $A$ --- non sono nel computo, perché computarli richiederebbe un funzionale globale sulla traiettoria che $\LP$ non possiede.

\emph{Disallineamento.} $\Lt$ inferisce un pianificatore dalla coerenza apparente delle sequenze; quando una mossa rompe la coerenza apparente, $\Lt$ non sa leggerla. La chiama ``stupidità'' o ``egocentrismo''. In realtà è la stessa scelta minima di sempre, vista nel suo limite strutturale: niente computa effetti a valle. La foto a $N$ rivela $\LP$ esattamente perché \emph{non} regge la lettura strategica.

\subsection*{Il compleanno: $T$ con $N$ ``per dimostrazione''}

\emph{Letta in $\Lt$.} Scena costruita per $A$. Lei tradisce $T$ con $N$ in modo lampante perché $A$ veda. ``Guarda di cosa sono capace, guarda chi mi sta accanto adesso, guarda cosa avresti potuto avere.'' Pianificatore-per-me al suo apice.

\emph{Prodotta in $\LP$.} Configurazione del pool esposta. $T$ accanto ($\mathrm{autonomy} \approx 0$, sotto threshold), $N$ in alto in classifica per accumulazione di $\Sogno$ e bassa threat, $A$ spettatore (high threat, $\Sogno(A)$ filtrato in ingresso dal $\lambda \gg 1$). $\LP$ non costruisce scene per destinatari --- non ha la grammatica per farlo. Espone la configurazione corrente, e basta.

\emph{Disallineamento.} $\Lt$ presuppone un destinatario calibrato; $\LP$ non ha modulo che modelli destinatari. La leggibilità del compleanno come ``scena per $A$'' è lettura di $\Lt$, non atto di $G$. La non-reazione di $A$ a quella sera --- pizza, festeggiamenti, ritorno da Sara --- non è equanimità conquistata: è che la configurazione era già stata letta (la foto a Canottaggio l'aveva esposta settimane prima). Nuova manifestazione, niente di nuovo per chi sapeva leggere.

\bigskip

Nessuno di questi episodi richiede strategia, malafede o calcolo. Sono esecuzione regolare di una legge applicata passo per passo a un pool che evolve. La coerenza apparente della sequenza --- che da $\Lt$ sembra disegno --- è proprietà interna della legge stessa, non firma di un'intenzione che la precede. Quando la coerenza si rompe (la foto a $N$), $\LP$ non si rompe: si vede.
